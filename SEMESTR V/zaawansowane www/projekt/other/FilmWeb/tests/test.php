...
<?php
    ...
    $filmweb->Login('androidmarket137@gmail.com ', 'qweqwe123')
        ->execute();
?>

<pre><?php var_dump($filmweb->getFilmInfoFull(491120)->execute()); ?></pre>
<pre><?php var_dump($filmweb->getUserFilmVotes(2436820, 0)->execute()); ?></pre>

<h2>Obsada filmu</h2>
<?php foreach(nSolutions\Filmweb::$roles as $type => $role):?>
<h3><?php echo $role; ?></h3>
<?php $casts = $filmweb->getFilmPersons(491120, $type, 0)->execute(); ?>
<?php if($casts):?>
