...
--
-- Host: 127.0.0.1
-- Czas generowania: 05 Sty 2016, 15:13
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_movieAwards` (IN `idMovie` INT(11))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_movieWhereBeActor` (IN `idPerson` INT(11))  NO SQL
BEGIN

SELECT m.id, m.poster, m.title, c.participation  FROM `creator` c, `person` p, `movie` m WHERE p.id = idPerson AND p.id = c.idPerson AND c.idMovie = m.id AND c.creator = 0;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `p_movieWhereBeCreator` (IN `idPerson` INT)  NO SQL
BEGIN
SELECT m.id, m.poster, m.title, c.participation FROM `creator` c, `person` p, `movie` m WHERE p.id = idPerson AND p.id = c.idPerson AND c.idMovie = m.id AND c.creator = 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `p_person` (IN `id` INT(11))  NO SQL
BEGIN 
	SELECT * FROM person p WHERE p.id = id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `p_personAwards` (IN `idPerson` INT)  NO SQL
BEGIN

SELECT * FROM `award` a WHERE a.idPerson = idPerson ORDER BY a.date, a.won DESC;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `p_persons` (IN `_from` INT(11), IN `_number` INT(11))  NO SQL
BEGIN
	SELECT * FROM person p ORDER BY p.name ASC LIMIT _from, _number;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `p_personsSearch` (IN `_from` INT, IN `_number` INT, IN `_search` VARCHAR(255) CHARSET utf8)  NO SQL
BEGIN 

SELECT * FROM person p WHERE p.name LIKE CONCAT('%',_search,'%') ORDER BY p.name ASC LIMIT _from, _number;
...
CREATE TABLE `award` (
  ...
  `won` text NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='tabela trzyma informacje o nagrodach. Jeśli otrzymał film idUser jest puste...';
...
CREATE TABLE `comment` (
  ...
  `description` varchar(255) NOT NULL COMMENT 'opis słowny komentarza',
  `date` varchar(25) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='informacje o komentarzach zostawionych przez użytkowników pod filmami ';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='tabela zawiera połączenie między komentarzami użytkowników a filmami';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='zawiera info o twórcach filmu jak i ich wkładzie do stworzenia filmu';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='tabela trzyma informacje o filmach';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='zawiera informacje o twórcach(włączając w to aktorów) filmów';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='informacje o użytkownikach zarejestrowanych';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='tabela trzyma informacje o czasie logowania się użytkownika do aplikacji';
...
--
ALTER TABLE `comment`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26306;
--
